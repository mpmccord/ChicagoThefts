...
zipChicagoDf <- get_acs(geography = 'zcta', variables = c( total_pop = "B01003_001"),year = i, geometry = FALSE, survey="acs5") %>%
...
bay_area_multi_year <- map_dfr(
  years,
  ~ get_acs(geography = 'zcta', variables = c( total_pop = "B01003_001"),year = i, geometry = FALSE, survey="acs5") %>%
                  select(GEOID, NAME, variable, estimate) %>% 
                  filter(str_detect( GEOID,"^606")) %>%  ## add a str filter
                  spread(variable, estimate),
  .id = "year"  # when combining results, add id var (name of list item)
