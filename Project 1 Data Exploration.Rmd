...
chicago_crime_monthly %>%
  autoplot(NumThefts)
  lambda <- thefts_ts |>
  features(TheftsPer1000, features = guerrero) |>
  pull(lambda_guerrero)
lambda
thefts_ts <- thefts_ts  %>%
  mutate(TheftsPer1000Transformed = box_cox(TheftsPer1000, lambda))
...
thefts_ts %>%
  autoplot(TheftsPer1000Transformed)
```
...
lambda <- chicago_crime_monthly |>
  features(NumThefts, features = guerrero) |>
  pull(lambda_guerrero)
thefts_train <- chicago_crime_monthly %>%
...
raw_count.model.forecast <- raw_count.model %>%
  forecast(h = "3 years")
raw_count.model.forecast %>%
  autoplot(chicago_crime_monthly)
raw_count.model %>%
  gg_tsresiduals()
```
...
```{r}
raw_count.model.forecast %>%
  accuracy(chicago_crime_monthly)
```
